---
title: "Stage M2 BGE - Diversité génomique d’une population d’abeilles à miel sélectionnée"
...
Lien vers les scripts bash et commandes de la publication de Wragg et al., 2022 : <https://github.com/avignal5/SeqApiPop/tree/v1.5>

### VCF Quality plot

```{r}
#E756_BeeMuSe
setwd("~/Documents/Stage_NB/data/BeeMuSe_Wageningen_E756_17_03_2023_Alain/BeeMuSe_Wageningen_E756_17_03_2023_Alain/Genotyping_data/BeeMuSe/vcf")

my_vcf <- read.vcfR("E756_BeeMuSe.vcf", verbose = FALSE)
chrom <- create.chromR(name="Pfeiffer variants", vcf=my_vcf)
chrom <- proc.chromR(chrom, verbose=TRUE)

plot(chrom)

chromoqc(chrom, xlim=c(0, 1250000))
```

```{r}
#setwd("~/Documents/Stage_NB/data/BeeMuSe_Wageningen_E756_17_03_2023_Alain/BeeMuSe_Wageningen_E756_17_03_2023_Alain/Genotyping_data/BeeMuSe/vcf")
setwd("~/Documents/Stage_NB/data")

#my_vcf <- read.vcfR("E756_BeeMuSe.vcf", verbose = FALSE)
my_vcf <- read.vcfR("subset_RefPop_samples_ref.vcf", verbose = FALSE)
chrom <- create.chromR(name="Pfeiffer variants", vcf=my_vcf)
chrom <- proc.chromR(chrom, verbose=TRUE)
...
> ``` bash
> for chromosome in NC_037638.1 NC_037639.1 NC_037640.1 NC_037641.1 NC_037642.1 NC_037643.1 NC_037644.1 NC_037645.1 NC_037646.1 NC_037647.1 NC_037648.1 NC_037649.1 NC_037650.1 NC_037651.1 NC_037652.1 NC_037653.1 NC_001566.1; do
> count=$(grep -c "^$chromosome" E756_BeeMuSe.vcf)
> echo "$chromosome: $count SNPs"
> done
>
> >E756_BeeMuSe.vcf
> NC_037638.1: 1514 SNPs
> NC_037639.1: 864 SNPs
> NC_037640.1: 764 SNPs
> NC_037641.1: 722 SNPs
> NC_037642.1: 750 SNPs
> NC_037643.1: 998 SNPs
> NC_037644.1: 740 SNPs
> NC_037645.1: 712 SNPs
> NC_037646.1: 667 SNPs
> NC_037647.1: 651 SNPs
> NC_037648.1: 876 SNPs
> NC_037649.1: 620 SNPs
> NC_037650.1: 620 SNPs
> NC_037651.1: 587 SNPs
> NC_037652.1: 523 SNPs
> NC_037653.1: 385 SNPs
> NC_001566.1: 7 SNPs
> ```

### Création fichier des RefPop Samples 'Diversity_Study_RefPop_Samples.txt'
...
# Afficher la liste des noms de samples
...
### Création fichier SeqApiPop Samples sans Ouessant / Colonsay
...
### Création fichier ind2pop_merged_data 3848 SNPs
...
# A CONTINUER CORRESPONDANCE BEEMUSE 
...
#colnames(ind2pop_merged_data_2)[colnames(ind2pop_merged_data_2) == "V1"] <- "name"
#ind_2_pop_merged_data_2_seq_api_labels <- merge(ind2pop_merged_data_2, seq_api_labels, by = "name")
```
...
print(anti_join(eigenvec, Corres_ID_E756_eigenvec, by = "name"))
#plq8-30 -> plq8-96
# => 67 samples not in Corres_ID_E756_eigenvec

print(anti_join(Corres_ID_E756_eigenvec, input_pedigree_BeeMuSe, by = "ID_1a"))
# => 69 samples not in input_pedigree_BeeMuSe
...
eigen_percent
#Soit 33.73% de la variation totale expliquée par la première composante principale et 22.80% par la deuxième composante principale.
#(filtrée) Soit 34.46% de la variation totale expliquée par la première composante principale et 22.85% par la deuxième composante principale.
...
ggplot(eigenvec, aes(x = V3, y = V4, label = Sample, color = Sample)) +
  geom_point() +
  labs(title = "PCA Plot", x = "PC1 : 34.46 % of variance", y = "PC2 : 22.85 % of variance") +
  theme(legend.position = "right")  
```

```{r}
  #plot ID_breeder : identification de l'apiculteur de chez qui viennent les abeilles
...
```{r}
ggplot(merged_3, aes(x = V3, y = V4, label = ID_2a, color = ID_2a)) +
  geom_point(size = 0.8) +  # Ajuster la taille des points selon vos préférences (par exemple, size = 2)
...
values_in_A_not_in_B # 136
```

###### Correspondance ID_2a - .FAM - BeeMuSe
...
#creer fichier correspondance beemuse ID_2a avec .fam
...
### ACP - Merged BeeMuSe SeqApiPop

#### ACP - SNPsBeeMuSe - filtered - 10030 SNPs

```{r}
setwd("~/Documents/Stage_NB/data/merged_BeeMuSe_SeqApiPop_629_filtered")
# Chargement des données
eigenvec_merged <- read.table("merged_BeeMuSe_SeqApiPop_629_filtered_acp.eigenvec", header = F) 
eigenval_merged <- read.table("merged_BeeMuSe_SeqApiPop_629_filtered_acp.eigenval", header = F) 
colnames(eigenvec_merged)[colnames(eigenvec_merged) == "V1"] <- "Sample"
colnames(eigenvec_merged)[colnames(eigenvec_merged) == "V2"] <- "name"
```

#### ACP - maf001 - LD03 - 3848 SNP
...
# Chargement des données
#eigenvec_merged_maf001_LD03 <- read.table("merged_BeeMuSe_SeqApiPop_629_filtered_maf001_LD03_acp.eigenvec", header = F) 
#eigenval_merged_maf001_LD03 <- read.table("merged_BeeMuSe_SeqApiPop_629_filtered_maf001_LD03_acp.eigenval", header = F) 
...
# Afficher les premières lignes de la nouvelle table
head(eigenvec_BeeMuSe_748_Samples)
```
...
#ACP plot Label colors
#custom_colors <- c("black", "mediumpurple4", "mediumpurple3", "lightskyblue", "mediumvioletred", "hotpink1", "yellow", "gold", "orange", "chocolate", "brown", "olivedrab3", "mediumseagreen", "red")
...
# Define custom colors for the first plot
...
```{r}
# SeqApiPop - 301 - beemuse - 612 - ID_2a - colors
...
# SeqApiPop - 301 - Label - colors _ lineages
custom_colors <- c( "black", "black", "black", 
...
plot1 + geom_point(data = eigenvec_merged_Corres_ID_E756_pedigree, aes(x = V3, y = V4, color = ID_2a),size=1) +
  ...
  #ID_2a beemuse 612 ID_2a seqapipop 301 

# SeqApiPop - 301 - Label - colors
...
```{r}
# Sample
plot1 <- ggplot(eigenvec_merged_maf001_LD03_seq_api_labels, aes(x = V3, y = V4), color = "black") +
  geom_point() +
  scale_y_reverse() 

plot1 + geom_point(data = eigenvec_merged_Corres_ID_E756_pedigree, aes(x = V3, y = V4, color = ID_breeder)) +
  labs(title = "PCA Plot - BeeMuSe & SeqApiPop", x = "PC1", y = "PC2") +
  annotation_custom(
    ggplotGrob(
      ggplot(variance_df, aes(x = factor(PC), y = Variance)) +
...
plot1 + geom_point(data = eigenvec_merged_Corres_ID_E756_pedigree, aes(x = V3, y = V4, color = Sample)) +
  labs(title = "PCA Plot - BeeMuSe & SeqApiPop", x = "PC1", y = "PC2") +
  annotation_custom(
    ggplotGrob(
      ggplot(variance_df, aes(x = factor(PC), y = Variance)) +
...
# Add BeeMuSe
plot1 + geom_point(data = eigenvec_BeeMuSe_748_Samples, aes(x = V5, y = V6), color = "red", size = 1) +  # Change the size here
...
#### ACP RefPop samples (filtered) - (LD03 pruned)

```{r}
# ACP - reference population samples 

#eigenvec_refpop <- read.table("subset_RefPop_samples.eigenvec", header = F)
#eigenval_refpop <- read.table("subset_RefPop_samples.eigenval", header = F)
eigenvec_refpop <- read.table("subset_RefPop_samples_filtered_acp.eigenvec", header = F)
eigenval_refpop <- read.table("subset_RefPop_samples_filtered_acp.eigenval", header = F)
...
eigen_percent_refpop
```

```{r}
#tree
#matrice_app_refpop <- read.table("subset_RefPop_samples.rel", header = FALSE)
#matrice_app_refpop <- read.table("subset_RefPop_samples_filtered_acp.rel", header = FALSE)
matrice_app_refpop <- read.table("subset_RefPop_samples_ref_LD03_acp.rel", header = FALSE)
  ...
  annotation_custom(
    ggplotGrob(
      ggplot(variance_df, aes(x = factor(PC), y = Variance)) +
        ...
        theme_minimal() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none")
    ), xmin = 0.073, xmax = 0.03, ymin = -0.15, ymax = -0.05)
```
...
ggplot(data = eigenvec_refpop_seq_api_labels, aes(x = V3, y = V4, color = Label)) +
  ...
  geom_text(aes(x = 0.035, y = 0.06, label = "C-lineage"), size = 4, color = "black") +
  geom_text(aes(x = -0.05, y = 0.04, label = "M-lineage"), size = 4, color = "black") +
  geom_text(aes(x = 0.045, y = -0.22, label = "O-lineage"), size = 4, color = "black") +
  labs(title = "PCA Plot - reference populations", x = "PC1", y = "PC2") +
  scale_color_manual(values = custom_colors_label2, 
                     breaks = c("Ouessant Conservatory", "Colonsay Conservatory", "Iberiensis Spain", 
                                "Savoy Conservatory", "Porquerolles Conservatory", "Sollies Conservatory", 
...
#### ACP - maf001 - LD default - 50 10 0.1

```{r}
#SNPsBeeMuSe / filtered / maf001 / LD03 default 50 10 0.1
setwd("~/Documents/Stage_NB/data/SeqApiPop_629_SNPsBeeMuSe")
  ...
  annotation_custom(
    ggplotGrob(
      ggplot(variance_df, aes(x = factor(PC), y = Variance)) +
        ...
        theme_minimal() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none")
    ), xmin = 0.028, xmax = 0.052, ymin = 0.04, ymax = 0.12)
...
### ACP - SeqApiPop - 561 - maf001 - rtgdg
...
### CV error plot - SeqApiPop - 629 - maf001
...
### CV error plot - SeqApiPop - 629 - SNPsBeeMuSe
...
#### CV error plot - LD03 default (50 10 0.1) - 1055 SNPs
...
### CV error plot - SeqApiPop - 561 - SNPsBeeMuSe

#### CV error plot - filtered - maf001 - LD03 - SNPs
...
#### CV error plot - filtered - maf001 - LD default (50 10 0.1) - 1055 SNPs
...
### CV error plot - Merged BeeMuSe SeqApiPop

#### CV error plot - filtered - maf001 - LD03
...
#### **CV error plot - 3848 - 629 - 30 runs**
...
#### CV error plot - 3848 - 561 - 30 runs
...
#### CV error plot - 1055 - 561 - 30 runs
...
#### CV error plot - 3848 SNPs - 629 - LD default - supervised
...
#### CV error plot - 1055 SNPs - 561 - LD default - supervised
...
### CV error plot - BeeMuSe only 3848 SNPs
...
### CV error plot - SeqApiPop - 561 - maf001
...
### CV plot légende de figure Admixture

#### SeqApiPop 629 maf001 LD03 - 248,183 SNPs
...
#### SeqApiPop 629 SNPsBeeMuSe - 10,030 SNPs
...
#### SeqApiPop 629 SNPsBeeMuSe maf001 50 10 0.1 - 1,055 SNPs
...
# Read the file
Q1 <- read.table("SeqApiPop_629_SNPsBeeMuSe_filtered_maf001_LD03_pruned.3.r0.Q", header = FALSE)

colnames(Q1)[colnames(Q1) == "V1"] <- "Noir"
colnames(Q1)[colnames(Q1) == "V2"] <- "Vert"
colnames(Q1)[colnames(Q1) == "V3"] <- "Orange"

# Create an empty vector to store the category for each row
categories <- character(nrow(Q1))

# Iterate through each row
for (i in 1:nrow(Q1)) {
  # Check if any value in the row is greater than 0.95
  if (any(Q1[i,] > 0.95)) {
    # Check which column has the value greater than 0.95
    if (Q1[i,1] > 0.95) {
      categories[i] <- "Noir"
    } else if (Q1[i,2] > 0.95) {
      categories[i] <- "Vert"
    ...
    # If none of the columns have values greater than 0.95
    categories[i] <- "-"
  }
}

# Write the categories to a single list file
write(categories, file = "output_list_1.txt")
```
...
ind_pop <- readLines("output_list_1_merged.txt")

# Replace "Beemuse" with "-"
ind_pop <- gsub("Beemuse", "-", ind_pop)

# Write the modified content back to the file
writeLines(ind_pop, "merged_data.pop")
```
...
# Read the file
Q_5 <- read.table("SeqApiPop_629_maf001_LD03_pruned.5.r0.Q", header = FALSE)

colnames(Q_5)[colnames(Q_5) == "V1"] <- "Bleu"
colnames(Q_5)[colnames(Q_5) == "V2"] <- "Orange"
colnames(Q_5)[colnames(Q_5) == "V3"] <- "Noir"
colnames(Q_5)[colnames(Q_5) == "V4"] <- "Jaune"
colnames(Q_5)[colnames(Q_5) == "V5"] <- "Vert"

# Create an empty vector to store the category for each row
categories <- character(nrow(Q_5))

# Initialisation du vecteur de catégories
categories <- rep("-", nrow(Q_5))
...
for (i in 1:nrow(Q_5)) {
  ...
  if (all(Q_5[i,] <= 0.95)) {
    ...
    if (Q_5[i,1] > 0.95) {
      categories[i] <- "Bleu"
    } else if (Q_5[i,2] > 0.95) {
      categories[i] <- "Orange"
    } else if (Q_5[i,3] > 0.95) {
      categories[i] <- "Noir"
    } else if (Q_5[i,4] > 0.95) {
      categories[i] <- "Jaune"
    } else if (Q_5[i,5] > 0.95) {
      categories[i] <- "Vert"
    }
  }
}
...
# Write the categories to a single list file
write(categories, file = "output_list_K5.txt")
```
...
# Read the file content
output_list_K5_merged <- readLines("output_list_K5_merged.txt")

# Replace "Beemuse" with "-"
output_list_K5_merged <- gsub("Beemuse", "-", output_list_K5_merged)

# Write the modified content back to the file
writeLines(output_list_K5_merged, "merged_data_K5.pop")
```
...
# Read the file
Q_6 <- read.table("SeqApiPop_629_maf001_LD03_pruned.6.r15.Q", header = FALSE)

colnames(Q_6)[colnames(Q_6) == "V1"] <- "Vert"
colnames(Q_6)[colnames(Q_6) == "V2"] <- "Orange"
colnames(Q_6)[colnames(Q_6) == "V3"] <- "Jaune"
colnames(Q_6)[colnames(Q_6) == "V4"] <- "Rouge"
colnames(Q_6)[colnames(Q_6) == "V5"] <- "Noir"
colnames(Q_6)[colnames(Q_6) == "V6"] <- "Bleu"

# Create an empty vector to store the category for each row
categories <- character(nrow(Q_6))

# Initialisation du vecteur de catégories
categories <- rep("-", nrow(Q_6))
...
for (i in 1:nrow(Q_6)) {
  ...
  if (all(Q_6[i,] <= 0.95)) {
    ...
    if (Q_6[i,1] > 0.95) {
      categories[i] <- "Vert"
    } else if (Q_6[i,2] > 0.95) {
      categories[i] <- "Orange"
    } else if (Q_6[i,3] > 0.95) {
      categories[i] <- "Jaune"
    } else if (Q_6[i,4] > 0.95) {
      categories[i] <- "Rouge"
    } else if (Q_6[i,5] > 0.95) {
      categories[i] <- "Noir"
    } else if (Q_6[i,6] > 0.95) {
      categories[i] <- "Bleu"
    }
  }
}
...
# Write the categories to a single list file
write(categories, file = "output_list_K6.txt")
```

```{r}
setwd("~/Documents/Stage_NB/data/Qfiles")

output_list_K6_merged <- readLines("output_list_K6_merged.txt")

# Replace "Beemuse" with "-"
output_list_K6_merged <- gsub("Beemuse", "-", output_list_K6_merged)

# Write the modified content back to the file
writeLines(output_list_K6_merged, "merged_data_K6.pop")
```

#### Sans Colonsay/Ouessant
...
#### Création fichier FST ind2pop merged data (Noir/Orange)
